...
export class CircuitExplainer {
  ...
  async explainSelection(): Promise<void> {
    ...
    if (!selectedText) {
      void vscode.window.showWarningMessage('Select a schematic block or relevant text before running AI Explain Circuit.');
      return;
    }
    ...
    this.logger.show();
    void vscode.window.showInformationMessage('Circuit explanation written to the KiCad Studio output channel.');
  }
}
