...
#
# Examples:
#   pwsh -ExecutionPolicy Bypass -File .\scripts\capability-audit.ps1 -Repo helix
#   pwsh -ExecutionPolicy Bypass -File .\scripts\capability-audit.ps1 -Repo repo-steward
#   pwsh -ExecutionPolicy Bypass -File .\scripts\capability-audit.ps1 -All
#   pwsh -ExecutionPolicy Bypass -File .\scripts\capability-audit.ps1 -All -ShowDetails
#
...
function Get-CountOrStatus {
    ...
    if (-not $Result.ok) {
        ...
        if ($msg -match "disabled") {
            return [pscustomobject]@{ status = "disabled"; count = $null; message = $DisabledHint }
        }
        if ($msg -match "no analysis found") {
            return [pscustomobject]@{ status = "no_analysis"; count = $null; message = "No analysis found" }
        }
        if ($msg -match "404") {
            return [pscustomobject]@{ status = "unavailable_or_disabled"; count = $null; message = $msg }
        }
        if ($msg -match "403") {
            return [pscustomobject]@{ status = "forbidden"; count = $null; message = $msg }
        }
        return [pscustomobject]@{ status = "error"; count = $null; message = $msg }
    }

    if ($null -eq $Result.data) {
        return [pscustomobject]@{ status = "ok"; count = 0; message = "" }
    }

    if ($Result.data -is [array]) {
        return [pscustomobject]@{ status = "ok"; count = $Result.data.Count; message = "" }
    }
...
function Test-Label {
    ...
    $raw = gh label list --repo $FullName --limit 300 --json name 2>$null
    if ($LASTEXITCODE -ne 0) {
        return $false
    }
    ...
    try {
        ...
        return [bool]($labels | Where-Object { # capability-audit.ps1
# Audits GitHub capabilities for repositories in an organization.
#
# Examples:
#   pwsh -ExecutionPolicy Bypass -File .\scripts\capability-audit.ps1 -Repo helix
...
function Test-Label {
    ...
    $null = gh label view $Label --repo $FullName 2>$null
...
function Get-ActiveFailedRuns {
    ...
    if ($LASTEXITCODE -ne 0) {
        return [pscustomobject]@{
            ok = $false
            activeFailed = @()
            ignoredClosedPrFailures = 0
            ignoredResolvedMainFailures = 0
        }
    }

    $runs = @($raw | ConvertFrom-Json)
    $branchMap = @{}
    foreach ($b in $OpenPrBranches) {
        if ($b) { $branchMap[$b] = $true }
    }
...
if ($userResult.ok -and $userResult.data) {
    $userPlan = $userResult.data.plan.name
    Write-Host "Active GitHub user: $($userResult.data.login)"
    Write-Host "GitHub plan:        $userPlan"
} else {
...
foreach ($target in $targets) {
    ...
    foreach ($label in $requiredLabels) {
        $labelStatus[$label] = Test-Label -FullName $fullName -Label $label
    }
    ...
    $missingLabels = @($requiredLabels | Where-Object { -not $labelStatus[$_] })
    if ($missingLabels.Count -gt 0) {
        Write-Host "missing labels:         $($missingLabels -join ', ')" -ForegroundColor DarkYellow
    }

    if ($runInfo.activeFailed.Count -gt 0 -and $ShowDetails) {
        Write-Host "active failed run details:" -ForegroundColor Red
        foreach ($run in $runInfo.activeFailed) {
            Write-Host "  - #$($run.databaseId) [$($run.conclusion)] $($run.name) on $($run.headBranch)"
            Write-Host "    $($run.url)"
        }
    }
...
.name -eq $Label } | Select-Object -First 1)
    } catch {
        return $false
    }
}
